...
const Sidebar = () => {
  ...
  const isSpot = location.pathname.startsWith("/docs/spot");

  const [openSection, setOpenSection] = useState(
    isSpot ? "spot_market" : "market"
  );
  const [openSubSection, setOpenSubSection] = useState(null);

  useEffect(() => {
    if (isSpot) {
      if (!openSection || !openSection.startsWith("spot_")) {
        setOpenSection("spot_market");
      }
    } else {
      if (!openSection || openSection.startsWith("spot_")) {
        setOpenSection("market");
      }
    }
  }, [isSpot]);
  ...
  return (
    <aside className={`sidebar ${collapsed ? "collapsed" : ""}`}>
      <div className="sidebar-scroll">
        ...
        {/* Guide & Status */}
        <NavLink to={isSpot ? "/docs/spot" : "/"} className="sidebar-link" end>Integration Guidance</NavLink>
        <NavLink to={isSpot ? "/docs/spot/system-status" : "/docs/v5/system-status"} className="sidebar-link">Get System Status Guide</NavLink>
        <NavLink to={isSpot ? "/docs/spot/rate-limit" : "/docs/v5/rate-limit"} className="sidebar-link">Rate Limit Rules</NavLink>
        <NavLink to={isSpot ? "/docs/spot/enum" : "/docs/v5/enum"} className="sidebar-link">Enums Definitions</NavLink>
        <NavLink to={isSpot ? "/docs/spot/error" : "/docs/v5/error"} className="sidebar-link">Error Codes</NavLink>

        {isSpot ? (
          <>
            {/* Spot Market (Public APIs) */}
            <div className="sidebar-section collapsible sidebar-link" onClick={() => toggleSection("spot_market")}>
              <span>Market</span><IoIosArrowForward className={openSection === "spot_market" ? "rotate" : ""} />
            </div>
            {openSection === "spot_market" && (
              <div className="sidebar-sub">
                ...
                <NavLink to="/docs/spot/market/candles" className="sidebar-link">Get Candlestick Data</NavLink>
              </div>
            )}
            ...
            <div className="sidebar-section collapsible sidebar-link" onClick={() => toggleSection("spot_ws")}>
              <span>Websocket</span><IoIosArrowForward className={openSection === "spot_ws" ? "rotate" : ""} />
            </div>
            {openSection === "spot_ws" && (
              <div className="sidebar-sub">
                <div className="sidebar-section collapsible sidebar-link sub-header" onClick={(e) => toggleSubSection(e, "spot_ws_public")}>
                  <span>Public</span><IoIosArrowForward className={openSubSection === "spot_ws_public" ? "rotate" : ""} />
                </div>
                {openSubSection === "spot_ws_public" && (
                  <div className="sidebar-sub deep">
                    <NavLink to="/docs/spot/websocket/public/market-channel" className="sidebar-link">Ticker Channel</NavLink>
                    <NavLink to="/docs/spot/websocket/public/candlestick-channel" className="sidebar-link">Candlestick Channel</NavLink>
                    <NavLink to="/docs/spot/websocket/public/trading-channel" className="sidebar-link">Public Recent Channel</NavLink>
                    <NavLink to="/docs/spot/websocket/public/depth-channel" className="sidebar-link">Depth Channel</NavLink>
                  </div>
                )}
              </div>
            )}
