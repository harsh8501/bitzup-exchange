    ...
    <div className="api-doc-container">
      ...
      <div className="docs-panel" ref={contentRef}>
        <div className="breadcrumb">
          <span>Position</span> <IoIosArrowForward size={12} /> <span className="pill">Set Trading Stop</span>
        </div>
        <h1 className="api-title">Set Trading Stop</h1>
        <p style={{ color: "#888", marginBottom: "40px" }}>Set take profit, stop loss, and trailing stop for your positions.</p>
        ...
        <div className="api-box">
          ...
          <div className="tree-view">
            {[
              ...
              { name: "tpTriggerBy", type: "string", desc: "TP trigger price type", values: ["LastPrice", "MarkPrice", "IndexPrice"] },
              { name: "slTriggerBy", type: "string", desc: "SL trigger price type", values: ["LastPrice", "MarkPrice", "IndexPrice"] },
              { name: "trailingStop", type: "string", desc: "Trailing stop distance" },
              { name: "positionIdx", type: "integer", req: true, desc: "Position index. 0: One-Way, 1: Buy side, 2: Sell side", values: ["0", "1", "2"] },
              { name: "tpOrderType", type: "string", desc: "TP order type", values: ["Market", "Limit"] },
              { name: "slOrderType", type: "string", desc: "SL order type", values: ["Market", "Limit"] },
            ].map(p => (
              <div className="tree-item" key={p.name}>
                <span className="p-name">{p.name}</span>
                <span className="p-type">{p.type}</span>
