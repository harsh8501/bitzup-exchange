    ...
    <div className="api-doc-container">
      ...
      <div className="docs-panel" ref={contentRef}>
        <div className="breadcrumb">
          <span>Asset</span> <IoIosArrowForward size={12} /> <span className="pill">Create Internal Transfer</span>
        </div>
        <h1 className="api-title">Create Internal Transfer</h1>
        <p style={{ color: "#888", marginBottom: "40px" }}>Transfer funds between different account types within your account.</p>
        ...
        <div className="api-box">
          ...
          <div className="tree-view">
            {[
              { name: "transferId", type: "string", req: true, desc: "Unique UUID for the transfer" },
              { name: "coin", type: "string", req: true, desc: "Coin name" },
              { name: "amount", type: "string", req: true, desc: "Transfer amount" },
              { name: "fromAccountType", type: "string", req: true, desc: "Source account type", values: ["UNIFIED", "CONTRACT", "SPOT", "OPTION", "INVESTMENT", "FUND"] },
              { name: "toAccountType", type: "string", req: true, desc: "Target account type", values: ["UNIFIED", "CONTRACT", "SPOT", "OPTION", "INVESTMENT", "FUND"] },
            ].map(p => (
              <div className="tree-item" key={p.name}>
                <span className="p-name">{p.name}</span>
                <span className="p-type">{p.type}</span>
