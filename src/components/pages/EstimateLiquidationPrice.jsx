...
export const EstimateLiquidationPrice = () => {
  ...
  "data": {
    ...
    const codeMap = {
        HTTP: `POST /futures/api/v1/estimate-liquidation-price HTTP/1.1
...
url = "https://test.bitzup.com/futures/api/v1/estimate-liquidation-price"
...
public class EstimateLiquidationPriceExample {
    public static void main(String[] args) throws Exception {
        ...
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create("https://test.bitzup.com/futures/api/v1/estimate-liquidation-price"))
...
async function estimateLiquidationPrice() {
  try {
    const response = await axios.post("https://test.bitzup.com/futures/api/v1/estimate-liquidation-price",
      { symbol: "BTCUSDT", side: "Buy", price: 92000, qty: 0.05, leverage: 10 },
      { headers: { "Content-Type": "application/json", Authorization: "Bearer <your_token>" } });
    ...
    return (
        <><div className="container-fluid p-0"><div className="api-layout"><div className="row">
            <div className="col-lg-9 col-md-12 api-content" ref={contentRef}>
                <div className="breadcrumb mb-4"><span className="kline-market">Private</span><span className="mx-2"><IoIosArrowForward className="kline-arrow" /></span><span className="pill">Estimate Liquidation Price</span></div>
                <h1 className="api-title">Estimate Liquidation Price</h1>
                <p className="api-desc">Estimate the liquidation price for a position before opening it. Useful for risk management calculations. The calculation uses the maintenance margin rate (MMR) for the given position value.</p>
                ...
                <div className="http-path"><span className="method post">POST</span><span className="path">/v1/estimate-liquidation-price</span></div>
                ...
                <div className="api-table-box"><table className="table table-striped api-table mb-0"><thead><tr><th>Parameter</th><th>Required</th><th>Type</th><th>Comments</th></tr></thead><tbody>
                    ...
                    <tr><td className="text-interval">side</td><td>true</td><td>string</td><td><span className="pill">Buy</span> (long) or <span className="pill">Sell</span> (short)</td></tr>
                    <tr><td className="text-interval">price</td><td>true</td><td>number</td><td>Entry price</td></tr>
                    <tr><td className="text-interval">qty</td><td>true</td><td>number</td><td>Position quantity</td></tr>
                    <tr><td className="text-interval">leverage</td><td>true</td><td>number</td><td>Leverage value</td></tr>
                </tbody></table></div>
                ...
                <div className="api-table-box"><table className="table table-striped api-table mb-0"><thead><tr><th>Parameter</th><th>Type</th><th>Comments</th></tr></thead><tbody>
                    <tr><td>liquidation_price</td><td>number</td><td>Estimated liquidation price</td></tr>
                </tbody></table></div>
